...
from sklearn.metrics import (
    confusion_matrix, f1_score, accuracy_score, classification_report, roc_curve, roc_auc_score
)

# Define file path
file_path = 'C:/Users/MADHAB MANNA/OneDrive/Desktop/Final lung cancer project/new dataset.csv'
...
if os.path.exists(file_path):
    ...
    sns.pairplot(
        dataset.select_dtypes(include=np.number).join(dataset['Result']),
        hue="Result", diag_kind="kde", markers=["o", "s"], palette="coolwarm"
    )
    ...
    def plot_spline(x, y, hue, xlabel, ylabel, title, data, colors):
        ...
        sns.lineplot(
            data=data, x=x, y=y, hue=hue, palette=colors, marker='o', linewidth=2
        )
    ...
    # Boxplot for Age distribution by Result
    plt.figure(figsize=(8, 6))
    sns.boxplot(
        data=dataset, x="Result", y="Age", hue="Result",
        palette={"Cancer": "red", "Non-Cancer": "green"}, legend=False
    )
    ...
    # Suppress UndefinedMetricWarning by handling labels with no predicted samples
    print("Confusion Matrix:")
    cm = confusion_matrix(Y_test, Y_pred, labels=['Non-Cancer', 'Cancer'])
    print(cm)

    # Detailed classification report with zero_division to handle undefined metrics
    ...
    if 'Cancer' in Y_test.values and 'Non-Cancer' in Y_test.values:
        ...
        print("ROC Curve could not be generated due to lack of predicted probabilities for both classes.")
else:
